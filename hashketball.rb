...
def big_shoe_rebounds
  bigShoe = []
    game_hash.each {|location, team_data |
   team_data[:players].values.each {|category|
          bigShoe.push(category[:shoe])
            if category[:shoe] == bigShoe.max
              return category[:rebounds]
            end
  }
